...
export function InsightsPage() {
  ...
  const hours = dryout?.hours ?? null;
  const confidence = dryout?.confidence ?? null;
    ...
    <div className="max-w-7xl mx-auto flex flex-col gap-12">
      ...
      <div className="grid grid-cols-1 md:grid-cols-12 gap-6">
        ...
        <div className="md:col-span-8 bg-surface-container-lowest rounded-xl p-8 md:p-10 ambient-shadow border border-surface-variant/40 flex flex-col justify-between relative overflow-hidden group">
          ...
          <div className="flex justify-between items-start relative z-10">
            ...
            {confidence != null && (
              <div className="flex items-center gap-1.5 px-4 py-2 bg-primary-container text-on-primary-container rounded-full">
                <Leaf size={18} />
                <span className="font-label-md text-label-md">Confidence: {Math.round(confidence * 100)}%</span>
              </div>
            )}
          </div>
          <div className="mt-12 mb-8 relative z-10">
            <p className="font-data-display text-[64px] md:text-[88px] leading-none text-primary tracking-tighter">
              {hours != null ? hours : '--'} <span className="text-[32px] md:text-[40px] text-primary/50 ml-2">hours</span>
            </p>
            <p className="font-headline-md text-headline-md text-secondary mt-4">until critical moisture threshold</p>
          </div>
          {hours == null && (
            <p className="font-body-md text-outline relative z-10">No forecast data yet. Start collecting sensor readings.</p>
          )}
        ...
        <div className="md:col-span-4 bg-tertiary-container text-on-tertiary-container rounded-xl p-8 flex flex-col justify-between ambient-shadow relative overflow-hidden">
          <div className="absolute right-0 bottom-0 opacity-10"><Droplets size={160} /></div>
          <div className="relative z-10">
            ...
            <p className="font-body-md text-body-md opacity-90 mb-6">
              {hours != null
                ? `Watering recommended within the next ${hours} hours to maintain ideal osmotic pressure.`
                : 'Sensor data pending. Recommendations will appear once readings are collected.'}
            </p>
          </div>
        </div>
        ...
        <div className="md:col-span-6 bg-surface-container-low rounded-xl p-8 border border-surface-variant/50 flex flex-col gap-6">
          <div className="flex items-center gap-3"><MemoryStick size={24} className="text-tertiary-container" /><h3 className="font-headline-md text-headline-md text-on-surface">Edge Intelligence</h3></div>
          ...
          <div className="flex flex-col gap-4 mt-2">
            <div className="flex items-start gap-4">
              <div className="w-8 h-8 rounded-full bg-primary-container/20 flex items-center justify-center shrink-0 mt-1"><WifiOff size={16} className="text-primary-container" /></div>
              <div><h4 className="font-label-md text-label-md text-on-surface mb-1">Local Processing</h4><p className="font-body-md text-body-md text-outline">No cloud dependency. Analyzes sensor data instantly on-device.</p></div>
            </div>
            <div className="flex items-start gap-4">
              <div className="w-8 h-8 rounded-full bg-tertiary-container/20 flex items-center justify-center shrink-0 mt-1"><BrainCircuit size={16} className="text-tertiary-container" /></div>
              <div><h4 className="font-label-md text-label-md text-on-surface mb-1">Continuous Learning</h4><p className="font-body-md text-body-md text-outline">Model weights adjust automatically after every actual watering cycle to improve accuracy.</p></div>
            </div>
          </div>
        </div>
        ...
        <div className="md:col-span-6 bg-surface-container-lowest rounded-xl p-8 border border-surface-variant/40 ambient-shadow">
          ...
          <div className="flex justify-center gap-6 mt-6">
            <div className="flex items-center gap-2"><div className="w-3 h-3 bg-outline-variant/50 rounded-sm" /><span className="font-label-md text-[12px] text-outline">Predicted Hours</span></div>
            <div className="flex items-center gap-2"><div className="w-3 h-3 bg-tertiary-container rounded-sm" /><span className="font-label-md text-[12px] text-outline">Actual Hours</span></div>
          </div>
        </div>
      </div>
    </div>
