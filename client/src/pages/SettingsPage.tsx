...
export function SettingsPage() {
  const [ssid, setSsid] = useState('Botanical_Zen_5G');
  const [wifiPass, setWifiPass] = useState('gardenpeace123');
  ...
  function handleWifiSubmit(e: React.FormEvent) {
    e.preventDefault();
    showToast('WiFi credentials updated');
    markChanged();
  }

  function handleTriggerReset() {
    showToast('Reset command sent to device');
  }

  function handleDiscard() {
    setSsid('Botanical_Zen_5G');
    setWifiPass('gardenpeace123');
    setMoistureThreshold(25);
    setTempThreshold(32);
    setFlowRate(2.5);
    setAutoAdjust(true);
    setChanged(false);
  ...
  return (
    <div className="zen-bg min-h-screen flex-1">
      ...
      <div className="flex flex-col gap-16 max-w-6xl w-full">
        ...
        <section className="flex flex-col gap-6">
          <div className="flex items-center gap-3 border-b border-surface-variant pb-4">
            <div className="w-10 h-10 rounded-full bg-surface-container-high flex items-center justify-center text-primary"><Wifi size={20} /></div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <div className="bg-surface/80 backdrop-blur-md rounded-xl p-8 ambient-shadow border border-surface-variant relative overflow-hidden">
              ...
              <form className="flex flex-col gap-6 relative z-10" onSubmit={handleWifiSubmit}>
                <div className="flex flex-col gap-2">
                  <label className="font-body-md text-body-md text-on-surface-variant" htmlFor="ssid">Network Name (SSID)</label>
                  <input id="ssid" type="text" value={ssid} onChange={e => { setSsid(e.target.value); markChanged(); }}
                ...
                <div className="flex flex-col gap-2">
                  ...
                  <input id="password" type="password" value={wifiPass} onChange={e => { setWifiPass(e.target.value); markChanged(); }}
                    className="zen-input w-full px-4 py-3 rounded-t-md font-body-md text-body-md text-on-surface" />
                </div>
                <button type="submit" className="mt-4 px-6 py-3 bg-primary-container text-on-primary-container rounded-full font-label-md text-label-md hover:scale-[1.02] transition-transform self-start shadow-sm">Update Credentials</button>
              </form>
            </div>
              ...
              <div className="bg-surface-container-low rounded-xl p-6 border border-surface-variant flex items-center justify-between">
                <div className="flex flex-col">
                  ...
                  <span className="font-body-lg text-body-lg text-primary flex items-center gap-2 mt-1">
                    <span className="w-3 h-3 rounded-full bg-secondary-container inline-block shadow-[0_0_8px_rgba(254,157,122,0.6)]" />Connected
                  </span>
                </div>
              ...
              <div className="bg-error-container/20 border border-error-container/50 rounded-xl p-6 flex flex-col gap-4">
                <div className="flex items-start gap-4">
                  ...
                  <div>
                    ...
                    <p className="font-body-md text-body-md text-on-surface-variant">Triggering a reset will temporarily disconnect your smart garden hub.</p>
                  </div>
                </div>
                <button type="button" onClick={handleTriggerReset}
                  className="mt-2 px-6 py-3 bg-error text-on-error rounded-full font-label-md text-label-md hover:bg-error/90 transition-colors self-end shadow-sm">Trigger Reset</button>
              </div>
            </div>
          </div>
        </section>
        ...
        <section className="flex flex-col gap-6">
          <div className="flex items-center gap-3 border-b border-surface-variant pb-4">
            <div className="w-10 h-10 rounded-full bg-surface-container-high flex items-center justify-center text-primary"><Sliders size={20} /></div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            <div className="bg-surface/80 backdrop-blur-md rounded-[24px] p-6 ambient-shadow border border-surface-variant flex flex-col justify-between min-h-[220px]">
              ...
              <div className="flex-1 flex flex-col justify-center gap-2">
                <div className="flex items-end gap-2">
                  <input type="number" value={moistureThreshold} onChange={e => { setMoistureThreshold(Number(e.target.value)); markChanged(); }} min={0} max={100}
                    className="zen-input w-20 text-center font-data-display text-data-display text-primary bg-transparent pb-1 px-0" />
            ...
            <div className="bg-surface/80 backdrop-blur-md rounded-[24px] p-6 ambient-shadow border border-surface-variant flex flex-col justify-between min-h-[220px]">
              ...
              <div className="flex-1 flex flex-col justify-center gap-2">
                <div className="flex items-end gap-2">
                  <input type="number" value={tempThreshold} onChange={e => { setTempThreshold(Number(e.target.value)); markChanged(); }} min={-10} max={50}
                    className="zen-input w-20 text-center font-data-display text-data-display text-primary bg-transparent pb-1 px-0" />
            ...
            <div className="bg-surface-container-low rounded-[24px] p-6 border border-surface-variant flex flex-col justify-between min-h-[220px]">
              ...
              <div className="flex-1 flex flex-col justify-center gap-4">
                <div className="flex items-end gap-2">
                  <input type="number" step={0.1} value={flowRate} onChange={e => { setFlowRate(Number(e.target.value)); markChanged(); }}
                    className="zen-input w-24 text-center font-data-display text-data-display text-primary bg-transparent pb-1 px-0" />
                ...
                <div className="w-full h-1 bg-surface-variant rounded-full mt-2 relative">
                  <div className="absolute left-0 top-0 h-full bg-primary-container rounded-full" style={{ width: `${Math.min(flowRate * 18, 100)}%` }} />
                </div>
              </div>
            </div>
            <div className="md:col-span-2 lg:col-span-3 bg-tertiary-fixed/30 backdrop-blur-md rounded-xl p-6 border border-tertiary-fixed-dim flex items-center justify-between">
              ...
              <label className="flex items-center cursor-pointer relative">
                <input type="checkbox" checked={autoAdjust} onChange={() => { setAutoAdjust(!autoAdjust); markChanged(); }} className="sr-only peer zen-toggle" />
        ...
        {changed && (
          <div className="sticky bottom-4 md:bottom-8 z-30 mt-8 w-full max-w-2xl mx-auto">
            <div className="bg-surface/90 backdrop-blur-xl border border-surface-variant rounded-full px-6 py-4 shadow-[0_10px_30px_rgba(23,49,36,0.1)] flex items-center justify-between">
              <span className="font-body-md text-body-md text-on-surface-variant flex items-center gap-2"><Info size={14} />Unsaved changes</span>
              <div className="flex gap-3">
                <button onClick={handleDiscard} className="px-5 py-2 text-primary font-label-md text-label-md hover:bg-surface-container rounded-full transition-colors">Discard</button>
                <button onClick={handleApply} className="px-6 py-2 bg-primary text-on-primary rounded-full font-label-md text-label-md hover:scale-[1.02] transition-transform shadow-md">Apply Changes</button>
              </div>
            </div>
          </div>
        )}
      ...
      {/* Toast */}
      {toast && (
        <div className="fixed bottom-24 right-8 z-50 bg-primary text-on-primary px-6 py-3 rounded-full shadow-lg font-label-md text-label-md">
          {toast}
        </div>
      )}
    </div>
